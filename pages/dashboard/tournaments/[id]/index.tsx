				...
				{!loading ? (
					<>
						<div className='mt-16 mr-4 pl-10'>
							...
							<p className='text-center'>{tournament_.name}</p>
							<p className='text-center'>
								Number of Participants: <span>{players && players.length}</span>
							</p>
							<p className='text-center'>
								Total Stake:{' '}
								<span className='font-bold'>{tournament_.totalStake} WCT</span>
							</p>
						</div>
						<div className='flex items-center justify-between text-[0.8rem]'>
							...
							<p>
								Deadline:{' '}
								<Moment unix format='YYYY-MM-DD HH:mm UTC'>
					...
					<ImSpinner9 className='animate-spin h-16 w-16 mr-auto ml-auto mt-48' />
				)}
			</div>
		</BasePageLayout>
	);
